...
class JomWebplayerModelCategory extends JomWebplayerModel {
	...
	function getparentid() {
		...
		if ($filter_category > -1) {
			$db     = JFactory::getDBO();
			$query  = "SELECT parent FROM #__jomwebplayer_category WHERE id=" . $filter_category;
			$db->setQuery($query);
			$output = $db->loadRow();
	...
	function gettotal() {
         ...
         $db               = JFactory::getDBO();
         $query            = "SELECT COUNT(*) FROM #__jomwebplayer_category";
		 $where            = array();
	...
	function getrow() {
         $db  = JFactory::getDBO();
         $row = JTable::getInstance('jomwebplayercategory', 'Table');
	...
	function save() {
	  ...
	  switch ($task) {
        case 'apply':
             $msg  = 'Changes Saved';
             $link = 'index.php?option=com_jomwebplayer&view=category&task=edit&'. JomWebplayerUtility::getToken() .'=1&'.'cid[]='.$row->id;
             break;
        case 'save':
        default:
              $msg  = 'Saved';
              $link = 'index.php?option=com_jomwebplayer&view=category';
              break;
        }
	...
	function delete() {
        ...
        if(count($cid)) {
            $query = "DELETE FROM #__jomwebplayer_category WHERE id IN ( $cids )";
            $db->setQuery( $query );
            if (!$db->query()) {
        ...
        $mainframe->redirect( 'index.php?option=com_jomwebplayer&view=category' );
	}
	...
	function move($inc) {
		...
		$mainframe->redirect( 'index.php?option=com_jomwebplayer&view=category', JText::_('New Ordering Saved') );
    }
	...
	function publish() {
        ...
        $reviewTable = JTable::getInstance('jomwebplayercategory', 'Table');
	...
	function getCategoryTree( $ids ) {
		...
		$array  = $ids;				
		while(count($array)) {
			$query = "SELECT id FROM #__jomwebplayer_category WHERE parent IN (".implode(',', $array).") AND id NOT IN (".implode(',', $array).")";
			$db->setQuery($query);
			$array = (JOMWEBPLAYER_JVERSION == '3.0') ? $db->loadColumn() : $db->loadResultArray();
