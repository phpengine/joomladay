...
class JomWebplayerViewVideo extends JomWebplayerView {
	...
	function generateBreadcrumbs( $params, $video ) {
		...
		if($video->category != 'none') {
			$db =  JFactory::getDBO();		
			$query = 'SELECT * FROM #__jomwebplayer_category WHERE name='. $db->quote( $video->category );
			$db->setquery($query);
			$row = $db->loadObject();		
			
			if ($row && !$row->parent == 0) {
				$query = 'SELECT * FROM #__jomwebplayer_category WHERE id='.$row->parent;
				$db->setquery($query);
				$parent_category = $db->loadObject();			
					
				if ($parent_category->parent != 0) {
					$query = 'SELECT * FROM #__jomwebplayer_category WHERE id='.$parent_category->parent;
					$db->setquery($query);
					$top_category = $db->loadObject();
				
					$crumbs[$index][0] = $top_category->name;
					$crumbs[$index][1] = JRoute::_('index.php?option=com_jomwebplayer&Itemid='.$Itemid.'&view=category'.$orderby.'&wid='.$top_category->id);
					$index++;	
				}
				$crumbs[$index][0] = $parent_category->name;
				$crumbs[$index][1] = JRoute::_('index.php?option=com_jomwebplayer&Itemid='.$Itemid.'&view=category'.$orderby.'&wid='.$parent_category->id);
				$index++;
			}
			
			if($row) {
        		$crumbs[$index][0] = $row->name;		
				$crumbs[$index][1] = JRoute::_('index.php?option=com_jomwebplayer&Itemid='.$Itemid.'&view=category'.$orderby.'&wid='.$row->id);
				$index++;
			}
		}
		
		$crumbs[$index][0] = $video->title;
		$crumbs[$index][1] = JRoute::_('index.php?option=com_jomwebplayer&Itemid='.$Itemid.'&view=video'.$orderby.'&wid='.$video->id);		
