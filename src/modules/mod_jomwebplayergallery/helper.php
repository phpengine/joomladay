...
class modjomwebplayergalleryHelper {
    ...
    public static function getItems( $params, $catid ) {
		...
		if($itm['type'] == 'Category' && !JRequest::getCmd('catid')) {			
			$query  = "SELECT * FROM #__jomwebplayer_category WHERE published=1";
			$query .= ($category != '') ? ' AND name IN ("'.$category.'")' : '';
		} else {
			$query = modjomwebplayergalleryHelper::buildQuery('videos', $category, $itm["type"]);       		
		}
	...
	public static function getPagination( $params, $catid ) {
		...
		if($params->get('type') == 'Category' && !JRequest::getCmd('catid')) {			
			$query  = "SELECT COUNT(id) FROM #__jomwebplayer_category WHERE published=1";
			$query .= ($category != '') ? ' AND name IN ("'.$category.'")' : '';
		} else {
			$query = modjomwebplayergalleryHelper::buildQuery('count', $category, $params->get('type'));       		
		}
	...
	public static function buildQuery($type, $category, $orderby) {
		if($type == 'count') {
			$query = "SELECT COUNT(id) FROM #__jomwebplayer_videos WHERE published=1";
		} else {
	...
	public static function getCategory() {
		 if($id = JRequest::getCmd('catid')) {
		 	$db     = JFactory::getDBO();
         	$query  = "SELECT name FROM #__jomwebplayer_category WHERE id=" . $db->quote( $id );
         	$db->setQuery( $query );
         	$output = $db->loadObjectList();
